...
void pythagoreanQR(const Eigen::MatrixXd & S1, const Eigen::MatrixXd & S2, Eigen::MatrixXd & S); // In, in, out
                   ...
                   const Eigen::Matrix<Scalar,Eigen::Dynamic,Eigen::Dynamic> &S)
{
    ...
    Scalar part1, part2, part3;
    // TODO: Numerically stable version
    part1 = 0.5*((S.template triangularView<Eigen::Upper>().transpose().solve(x-mu)).squaredNorm()); // template for triangularView because the tip box said so
    part2 = n/2.0*std::log(2*M_PI); 
    part3 = S.diagonal().array().abs().log().sum(); 

    return -(part1 + part2 + part3);

    // Really bad version
    //Eigen::Matrix<Scalar,Eigen::Dynamic,Eigen::Dynamic> P = S.transpose()*S; // Bad, because unnecessary and loss of accuracy
    //Eigen::Matrix<Scalar,Eigen::Dynamic,Eigen::Dynamic> Pinv = P.inverse();  // Bad, because you should know better (https://www.johndcook.com/blog/2010/01/19/dont-invert-that-matrix/)
    //using std::log, std::sqrt, std::exp;
    //Scalar quadraticForm = (x - mu).transpose()*Pinv*(x - mu);
    //return log( 1.0/sqrt( (2*M_PI*P).determinant() )*exp(-0.5*quadraticForm) );
                   ...
                   Eigen::Matrix<Scalar,Eigen::Dynamic,1> &g)
{
    ...
    g = S.template triangularView<Eigen::Upper>().solve(S.template triangularView<Eigen::Upper>().transpose().solve(mu-x));
                   ...
                   Eigen::Matrix<Scalar,Eigen::Dynamic,Eigen::Dynamic> &H)
{
    ...
    H = -S.template triangularView<Eigen::Upper>().solve(
            S.transpose().template triangularView<Eigen::Lower>().solve(
                Eigen::Matrix<Scalar,Eigen::Dynamic,Eigen::Dynamic>::Identity(S.rows(),S.cols())
                )
            );
    ...
    )
{
    ...
    // Q: do we only affine transform the measurements? -> no, slide 32, transform all the things

    // Follow dat MATLAB algorithm  
    pythagoreanQR(Sxx*C.transpose(), SR, S);
    R = S.triangularView<Eigen::Upper>();
    Syy = R.topRows(ny);
